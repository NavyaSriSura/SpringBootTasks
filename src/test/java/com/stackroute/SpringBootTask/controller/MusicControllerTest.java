...
public class MusicControllerTest {
    ...
    public void saveMusicFailure() throws Exception {
        when(musicService.saveMusic(any())).thenThrow(TrackAlreadyExistsException.class);
        mockMvc.perform(MockMvcRequestBuilders.post("/api/v1/music")
                .contentType(MediaType.APPLICATION_JSON).content(asJsonString(music)))
    ...
    @Test
    public void testMusicControllerUpdate() throws Exception{
        when(musicService.UpdateById(music,1)).thenReturn(true);
        ...
        verify(musicService,times(1)).UpdateTrack(music,1);
        verifyNoMoreInteractions(musicService);
    }

    @Test
    public void testMusicControllerDelete() throws Exception{
        when(musicService.deleteById(1)).thenReturn(true);
        mockMvc.perform(delete("/api/v1/music/1").contentType(MediaType.APPLICATION_JSON_UTF8)
